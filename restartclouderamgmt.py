...
### Main function ###
def main():
   API = ApiResource(CM_HOST, version=15, username=ADMIN_USER, password=ADMIN_PASS)
   print "Connected to CM host on " + CM_HOST

   MGMT = API.get_cloudera_manager().get_service()

   print "About to restart cloudera management service."
   MGMT.restart().wait()
   print "Done restarting cloudera management service ."
